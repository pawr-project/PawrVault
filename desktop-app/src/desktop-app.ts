  ...
  constructor() {
    ...
    autoUpdater.on('update-available', (event, releaseNotes, releaseName) => {
      ...
      const dialogOpts = {
        ...
        title: 'New Version',
        message: 'An update for PawrVault is available!',
    ...
    autoUpdater.on('error', message => {
      ...
      mainWindow.setTitle(`PawrVault - ${autoUpdater.currentVersion}`); // reset title
      showUpdateErrors = false; // disable errors
      const dialogOpts = {
        type: 'error',
        buttons: ['OK'],
        title: 'Update Error',
        message: 'Something went wrong while downloading PawrVault.',
...
function createWindow () {
  ...
  mainWindow.webContents.on('did-finish-load', function () {
    mainWindow.setTitle(`PawrVault - ${autoUpdater.currentVersion}`);
  });
...
function sendStatusToWindow(progressObj) {
  ...
  mainWindow.setTitle(`PawrVault - ${autoUpdater.currentVersion} - Downloading Update: ${Math.round(progressObj.percent)} %`);
}
...
if (!appLock) {
  ...
  // Refocus the window if the user attempts to open PawrVault while it is already open
  ...
  const template: any = [
    ...
    },
    {
      ...
      submenu: [
        ...
        {
          label: 'View GitHub',
          click () { loadExternal('https://github.com/pawr-project/PawrVault'); }
        },
        {
          label: 'Submit a bug report',
          click () { loadExternal('https://github.com/pawr-project/PawrVault/issues/new'); }
        },
        {
          label: 'Release notes',
          click () { loadExternal('https://github.com/pawr-project/PawrVault/releases'); }
        },
        {type: 'separator'},
        {
